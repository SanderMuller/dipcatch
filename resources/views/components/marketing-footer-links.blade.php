...
<div class="border-t border-ink/10 pt-6">
    @if ($useCases !== [])
        <nav class="flex flex-wrap items-baseline gap-x-5 gap-y-2 text-sm text-zinc-500 dark:text-zinc-400" aria-label="{{ __('Price alerts by category') }}">
            ...
            @foreach ($useCases as $useCase)
                <a href="{{ route('use-case', [...$langQuery, 'slug' => $useCase->slug]) }}" class="hover:text-brand">{{ $useCase->label }}</a>
            @endforeach
        </nav>
    @endif
    ...
    @if ($shops !== [])
        <nav @class(['flex flex-wrap items-baseline gap-x-5 gap-y-2 text-sm text-zinc-500 dark:text-zinc-400', 'mt-4' => $useCases !== []]) aria-label="{{ __('Price alerts by shop') }}">
            <a href="{{ route('shops', $langQuery) }}" class="font-medium text-zinc-700 hover:text-brand dark:text-zinc-300">{{ __('Supported shops') }}</a>
            @foreach ($shops as $shop)
                <a href="{{ route('shop', [...$langQuery, 'slug' => $shop['slug']]) }}" class="hover:text-brand">{{ $shop['name'] }}</a>
            @endforeach
        </nav>
    @endif
    ...
    <div @class(['flex flex-col items-center justify-between gap-3 text-sm text-zinc-500 sm:flex-row dark:text-zinc-400', 'mt-6' => $useCases !== [] || $shops !== []])>
        ...
        <nav class="flex items-center gap-5" aria-label="{{ __('Footer') }}">
            @if ($home)
                <a href="{{ route('home', $langQuery) }}" class="hover:text-brand">{{ __('Back to the homepage') }}</a>
            @endif
            <a href="{{ route('pricing', $langQuery) }}" class="hover:text-brand">{{ __('Pricing') }}</a>
            <a href="{{ route('privacy', $langQuery) }}" class="hover:text-brand">{{ __('Privacy') }}</a>
            <a href="{{ route('terms', $langQuery) }}" class="hover:text-brand">{{ __('Terms') }}</a>
            <a href="{{ route('support', $langQuery) }}" class="hover:text-brand">{{ __('Support') }}</a>
            @if (filled($contactEmail))
                <a href="mailto:{{ $contactEmail }}" class="hover:text-brand">{{ __('Contact') }}</a>
            @endif
        </nav>
    </div>
</div>
