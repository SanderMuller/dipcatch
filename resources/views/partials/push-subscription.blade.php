{{--
    The browser-push lifecycle: permission, service-worker registration,
    subscription, and the server's rejection path. Extracted so the Flux
    settings page reuses it verbatim — rebuilding only the toggle would leave a
    saved preference with no working delivery channel.
--}}
    <div
        x-data="{
            ...
            disablePushToggle(reason) {
                ...
                if (this.$wire) {
                    this.$wire.set('notify_via_push', false);
                }
            },
        ...
        x-init="$watch('$wire.notify_via_push', (val) => { if (val) ensureSubscription(); })"
