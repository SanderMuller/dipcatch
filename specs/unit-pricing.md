...
- **Post-ship (2026-09-01).** Dirk and Lidl now resolve pack sizes after all: both hosts got dedicated adapters (`DirkAdapter`, `LidlAdapter`) that keep the JSON-LD price and read the authoritative pack size from the page's `__NUXT_DATA__` devalue payload. lidl.nl scrapes directly (the boodschaapje.nl fast-fail is gone), so Lidl gets live prices and unit prices; the Section 1 "Dirk mostly shows none" consequence no longer holds.
