...
- [x] Eye-verify in a browser against the real AH shop, which is in a bonus until 6 September: the row shows the window, the layout survives on a phone width, and no console errors appear.
...
- **Eye-verified 2026-09-03** on `/app/products/01a059c1-5380-707a-ae0b-d3724ed2f111`, once the user logged in. Desktop: `ah.nl EUR 1.69 — "VOOR 1.69 until 6 Sep"` (the shop's own label) and `spar.nl EUR 2.45 — "Bonus until 10 Sep"` (from `priceValidUntil`, generic label). The four shops that state no window show none, and the unit prices still render beside them. At 390px the compact column joins both on one line: `EUR 8.45 /kg · VOOR 1.69 until 6 Sep`. No console errors; the harness `console.mjs` reports `clean: true, navStatus 200`, no page errors and no failed requests.
