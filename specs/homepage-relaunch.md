...
- Phone mock: below `lg` it renders **under** the copy, centred, at `w-64`, without the `rotate-3` tilt (the tilt clips against the section edge at 390 px); above `lg` it keeps the current column and tilt. Each card shows three lines — product, drop line (old → new), unit-price line — and the browser check at 390 px asserts for every card that none of the three is clipped, ellipsised or overlapping at `w-64` (`scrollWidth === clientWidth` per line element; no `truncate` class on price lines). If a line does not fit, shorten the example copy (for example drop "Extra Belegen"), never the price.
